...
function areEqual(prev: ListProps, next: ListProps) {
  ...
  if (prev.animate !== next.animate) {
    return false;
  }
  if (prev.onPress !== next.onPress) {
    return false;
  }
  if (prev.onPressIn !== next.onPressIn) {
    return false;
  }
  if (prev.onPressOut !== next.onPressOut) {
    return false;
  }
  if (prev.style !== next.style) {
    return false;
  }
  if (prev.contentContainerStyle !== next.contentContainerStyle) {
    return false;
  }
  ...
  if (prev.accessibilityRole !== next.accessibilityRole) {
    return false;
  }
  if (prev.accessibilityLabel !== next.accessibilityLabel) {
    return false;
  }
  if (prev.accessibilityLabel !== next.accessibilityLabel) {
    return false;
...
const ItemComponent = React.forwardRef<typeof Pressable, ListProps>(
  function ItemComponent(
    ...
    const handlePressIn = useCallback(
      (event: any) => {
        ...
        // Subtle tactile feedback on every list row — system-wide polish.
        try {
          void hapticSelection();
        } catch {
          // best-effort, never block press
        }
